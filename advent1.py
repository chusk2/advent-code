...
"""
with open('advent1.txt', 'r', encoding='utf-8') as f:
    data = []
...
for i in range(len(data)-1):
    if data[i+1] > data[i]:
        increments += 1
print(f'There has been {increments} in the series.')
...
# figure out how many triplets can be formed
# divide the total number of depths between 3 and leave out the remainder of the
# division. e.g. 17 / 3 = 5.666 --> five triplets and 2 as remainder (17%5)

max_index = len(data) - (len(data) % 3)
triplets_sums = []
indexes = range(0,20,3)

for i in indexes:
    triplets_sums.append(sum(data[i:i+1]))

incremented_triplets = 0
for i in range(len(triplets_sums)-1):
    if triplets_sums[i+1] > triplets_sums[i]:
        incremented_triplets += 1

print(f'There have been {incremented_triplets} in the series.')
